...
单 Activity MVVM Android 应用（`com.dlam.textediting`），minSdk 24，targetSdk 34，compileSdk 36。Kotlin 2.1 + Jetpack Compose + Material 3。**精简版仅保留单文件打开/编辑/保存、行号显示与文本搜索**；不包含多标签页、文件树、语法高亮、文本统计、最近文件、自动保存或撤销/重做。
...
| `MainViewModel.kt` | 应用状态。文件 I/O（打开/保存/另存/新建）、文本搜索（上一个/下一个）。 |
| `MainScreen.kt` | 顶层 Compose UI。工具栏（可横向滚动）、搜索栏、包装 `LinedEditText` 的 `AndroidView`、跳转行/设置对话框。 |
| `editor/LinedEditText.kt` | 自定义 `AppCompatEditText`，带 Canvas 绘制的行号栏。仅绘制可见行 + 3 行缓冲，可复用 `CharArray` 零分配格式化行号。行号栏宽度在 `onLayout`（布局阶段）按总行数位数预计算（只增不减），绘制阶段只读、零分配，避免错位/抖动。垂直对齐以 `extendedPaddingTop` 为基准，滚动用 `postInvalidateOnAnimation` 刷新。 |
| `SettingsManager.kt` | SharedPreferences 包装器，以 `StateFlow` 形式暴露设置供响应式 UI 使用。设置项：字体大小（10-24sp）、行号开关、自动换行开关、主题模式（跟随系统/浅色/深色）。 |
| `dialogs/SearchBar.kt`（`EditorComponents.kt`） | 文本搜索栏：输入、匹配计数、上/下导航。 |
...
`FileTreePanel.kt`、`FileTreeState.kt`、`RecentFilesManager.kt`、`dialogs/StatsDialog.kt`、`editor/SyntaxHighlighter.kt`、`util/StatsComputer.kt`、`UndoManager.kt`，以及多标签页、文件树、全局搜索/替换、语法高亮、文本统计、最近文件、自动保存、撤销/重做、搜索过滤选项（大小写/全字）及其设置项。